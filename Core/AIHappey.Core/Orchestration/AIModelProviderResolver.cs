    ...
    IHttpClientFactory httpClientFactory) : IAIModelProviderResolver
{
        ...
        CancellationToken ct)
    {
        ...
        // ---- 3) INTERNAL ENRICHMENT (provider -> provider) ----
        // Fill missing metadata from other models with same base model id
        var modelsByBase = result.Values
            .Select(v => v.Item1)
            .GroupBy(m => m.Id.SplitModelId().Model, StringComparer.OrdinalIgnoreCase);
        ...
        foreach (var group in modelsByBase)
        {
            ...
            var tags = models.FirstOrDefault(m => m.Tags?.Count() > 0)?.Tags;
