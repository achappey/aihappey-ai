        ...
        return await _memoryCache.GetOrCreateAsync(
            ...
            async ct =>
            {
                ...
                if (!resp.IsSuccessStatusCode)
                {
                    ...
                    throw new Exception($"MumeAI API error: {err}");
                }
                ...
                foreach (var el in arr)
                {
                    ...
                    {
                        var inputPrice = pricingEl.TryGetProperty("prompt_per_million", out var inputEl) &&
                                         inputEl.ValueKind == JsonValueKind.Number
                            ? inputEl.GetDecimal() / 1_000_000m
                            : 0m;

                        var outputPrice = pricingEl.TryGetProperty("completion_per_million", out var outputEl) &&
