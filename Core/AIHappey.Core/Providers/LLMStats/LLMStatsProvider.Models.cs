...
public partial class LLMStatsProvider
{
        ...
        return await _memoryCache.GetOrCreateAsync(
            ...
            async ct =>
            {
                ...
                if (!resp.IsSuccessStatusCode)
                {
                    ...
                    throw new Exception($"LLMStats API error: {err}");
                }
                ...
                foreach (var el in arr)
                {
                    ...
                    if (el.TryGetProperty("organization_name", out var orgEl))
                        model.OwnedBy = orgEl.GetString() ?? "";

                    if (el.TryGetProperty("display_name", out var nameEl))
                        model.Name = nameEl.GetString() ?? model.Name;

                    var inputPrice = el.TryGetProperty("input_price", out var inEl) &&
                    inEl.ValueKind == JsonValueKind.Number
                    ? inEl.GetDecimal()
                    : (decimal?)null;

                    var outputPrice = el.TryGetProperty("output_price", out var outEl) &&
