...
public partial class ZaiProvider
{
        ...
        [EnumeratorCancellation] CancellationToken cancellationToken)
    {
        ...
        if (!TryResolveAgentId(request.Model, out var agentId))
            throw new NotSupportedException($"Unsupported Z.AI agent model '{request.Model}'.");

        if (agentId == ViduTemplateAgentId)
            throw new NotSupportedException("Z.AI vidu_template_agent creates asynchronous video tasks and is not supported on unified streaming.");

        var options = request.ToChatCompletionOptions(GetIdentifier());
        options.Stream = true;
        options.Store ??= false;
        EnsureAgentUserMessage(options, request);
        ...
        {
            var error = await resp.Content.ReadAsStringAsync(cancellationToken);
            throw new HttpRequestException($"Z.AI agent unified stream request failed ({(int)resp.StatusCode}): {error}");
        }

        await using var stream = await resp.Content.ReadAsStreamAsync(cancellationToken);
        using var reader = new StreamReader(stream);
        await using var captureSink = ProviderBackendCapture.BeginStreamCapture("agents", resp, capture);
        var state = new ZaiAgentUnifiedStreamState(GetIdentifier(), request.Model ?? options.Model, agentId, request.Id);
        var fallbackCompletionId = Guid.NewGuid().ToString("n");
        var fallbackEmittedAnyContent = false;
        var fallbackEmittedTerminalChunk = false;
        string? fallbackActiveSlideToolCallId = null;
        string? fallbackActiveSlideToolName = null;
        var fallbackSlideToolOrdinal = 0;
               ...
               && (line = await reader.ReadLineAsync(cancellationToken)) is not null)
        {
            ...
            {
                evtDoc = JsonDocument.Parse(data);
                var emittedFromDirectParser = false;
                foreach (var streamEvent in state.Process(evtDoc.RootElement))
                {
                    emittedFromDirectParser = true;
                    yield return streamEvent;
                }

                if (!emittedFromDirectParser)
                {
                    var fallbackUpdate = NormalizeAgentStreamResponse(
                        evtDoc.RootElement,
                        options.Model,
                        agentId,
                        ref fallbackCompletionId,
                        ref fallbackEmittedAnyContent,
                        ref fallbackEmittedTerminalChunk,
                        ref fallbackActiveSlideToolCallId,
                        ref fallbackActiveSlideToolName,
                        ref fallbackSlideToolOrdinal);

                    if (fallbackUpdate is not null)
                    {
                        foreach (var fallbackEvent in fallbackUpdate.ToUnifiedStreamEvents(GetIdentifier()))
        ...
        foreach (var streamEvent in state.Complete(state.HadContent ? "stop" : "network_error", null))
            yield return streamEvent;
    }

    private static void EnsureAgentUserMessage(ChatCompletionOptions options, AIRequest request)
    {
        if (options.Messages.Any())
            return;
        ...
        [
            ...
            {
                Role = "user",
                Content = JsonSerializer.SerializeToElement(request.Input.Text, AgentJson)
            }
        ];
    }*/
