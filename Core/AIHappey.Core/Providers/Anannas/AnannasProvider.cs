...
namespace AIHappey.Core.Providers.Anannas;

public partial class AnannasProvider : IModelProvider
{
    ...
    public AnannasProvider(IApiKeyResolver keyResolver, AsyncCacheHelper asyncCacheHelper,
        IHttpClientFactory httpClientFactory)
    {
        _keyResolver = keyResolver;
        _memoryCache = asyncCacheHelper;
        _client = httpClientFactory.CreateClient();
        _client.BaseAddress = new Uri("https://api.anannas.ai/");
    }
    ...
    private void ApplyAuthHeader()
    {
        ...
        if (string.IsNullOrWhiteSpace(key))
            throw new InvalidOperationException($"No {nameof(Anannas)} API key.");
    ...
    public string GetIdentifier() => nameof(Anannas).ToLowerInvariant();
