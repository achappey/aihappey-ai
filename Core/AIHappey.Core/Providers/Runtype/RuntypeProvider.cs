...
public sealed partial class RuntypeProvider : IModelProvider, IUnifiedModelProvider
{
    private readonly IApiKeyResolver _keys;
    private readonly AsyncCacheHelper _cache;
    private readonly HttpClient _client;

    public RuntypeProvider(IApiKeyResolver keys, AsyncCacheHelper cache, IHttpClientFactory clients)
    {
        _keys = keys;
        _cache = cache;
        _client = clients.CreateClient();
        _client.BaseAddress = new Uri("https://api.runtype.com/");
        _client.Timeout = Timeout.InfiniteTimeSpan;
    }

    public string GetIdentifier() => "runtype";

    private HttpRequestMessage CreateRequest(HttpMethod method, string path)
    {
        var key = _keys.Resolve(GetIdentifier());
        if (string.IsNullOrWhiteSpace(key))
            throw new InvalidOperationException("No Runtype API key.");
