...
public partial class JulesProvider : IModelProvider
{
    ...
    public JulesProvider(IApiKeyResolver keyResolver, AsyncCacheHelper asyncCacheHelper,
        IHttpClientFactory httpClientFactory)
    {
        _keyResolver = keyResolver;
        _memoryCache = asyncCacheHelper;
        _client = httpClientFactory.CreateClient();
        _client.BaseAddress = new Uri("https://jules.googleapis.com/v1alpha/");
    }
    ...
    private void ApplyAuthHeader()
    {
        ...
        if (string.IsNullOrWhiteSpace(key))
            throw new InvalidOperationException($"No {nameof(Jules)} API key.");
    ...
    public string GetIdentifier() => nameof(Jules).ToLowerInvariant();
