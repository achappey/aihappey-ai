...
public partial class XiaomiMIMOProvider : IModelProvider
{
    ...
    public XiaomiMIMOProvider(IApiKeyResolver keyResolver, AsyncCacheHelper asyncCacheHelper,
        IHttpClientFactory httpClientFactory)
    {
        _keyResolver = keyResolver;
        _memoryCache = asyncCacheHelper;
        _client = httpClientFactory.CreateClient();
        _client.BaseAddress = new Uri("https://api.xiaomimimo.com/");
    }
    ...
    private void ApplyAuthHeader()
    {
        ...
        if (string.IsNullOrWhiteSpace(key))
            throw new InvalidOperationException($"No {nameof(XiaomiMIMO)} API key.");
    ...
    public string GetIdentifier() => nameof(XiaomiMIMO).ToLowerInvariant();
