...
public partial class RequestyProvider
{
    ...
    private static decimal? TryGetUsageCost(object? usage)
    {
        ...
        if (usageElement.ValueKind != JsonValueKind.Object
            || !TryGetRequestyProperty(usageElement, "cost", out var costElement))
        {
            return null;
        }

        return TryGetRequestyDecimal(costElement);
    }

    private static decimal? TryGetRequestyDecimal(JsonElement element)
    {
    ...
    private static bool TryGetRequestyProperty(JsonElement element, string propertyName, out JsonElement value)
    {
