...
namespace AIHappey.Core.Providers.StandardCompute;

public partial class StandardComputeProvider : IModelProvider
{
    ...
    public StandardComputeProvider(IApiKeyResolver keyResolver, AsyncCacheHelper asyncCacheHelper,
        IHttpClientFactory httpClientFactory)
    {
        _keyResolver = keyResolver;
        _memoryCache = asyncCacheHelper;
        _client = httpClientFactory.CreateClient();
        _client.BaseAddress = new Uri("https://api.stdcmpt.com/");
    }
    ...
    private void ApplyAuthHeader()
    {
        ...
        if (string.IsNullOrWhiteSpace(key))
            throw new InvalidOperationException($"No {nameof(StandardCompute)} API key.");
    ...
    public string GetIdentifier() => nameof(StandardCompute).ToLowerInvariant();
