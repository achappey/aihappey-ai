...
public partial class Agent37Provider : IModelProvider
{
        ...
        IHttpClientFactory httpClientFactory)
    {
        ...
        _client.BaseAddress = new Uri("https://api.agent37.com/");
    }

    private void ApplyModelAuthHeader()
    {
        var key = _keyResolver.Resolve(GetIdentifier());

        if (string.IsNullOrWhiteSpace(key))
            throw new InvalidOperationException($"No {nameof(Agent37)} API key.");

        _client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", key);
    }

    private void ApplyResponsesAuthHeader()
    {
        var key = _keyResolver.Resolve(GetIdentifier());
        ...
        _client.DefaultRequestHeaders.Remove("X-Agent37-Key");
        _client.DefaultRequestHeaders.Add("X-Agent37-Key", key);
    }
    ...
    public string GetIdentifier() => nameof(Agent37).ToLowerInvariant();
