...
namespace AIHappey.Core.Providers.Pellet;

public partial class PelletProvider : IModelProvider
{
    ...
    public PelletProvider(IApiKeyResolver keyResolver, AsyncCacheHelper asyncCacheHelper,
        IHttpClientFactory httpClientFactory)
    {
        _keyResolver = keyResolver;
        _memoryCache = asyncCacheHelper;
        _client = httpClientFactory.CreateClient();
        _client.BaseAddress = new Uri("https://getpellet.io/");
    }
    ...
    private void ApplyAuthHeader()
    {
        ...
        if (string.IsNullOrWhiteSpace(key))
            throw new InvalidOperationException($"No {nameof(Pellet)} API key.");
    ...
    private static void ApplyRouterPreference(ChatCompletionOptions options)
    {
        ...
        options.AdditionalProperties["pellet_config"] =
            JsonSerializer.SerializeToElement(new
            {
                routing_mode = options.Model
            });
    ...
    public string GetIdentifier() => nameof(Pellet).ToLowerInvariant();
