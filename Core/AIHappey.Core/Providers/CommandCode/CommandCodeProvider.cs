...
namespace AIHappey.Core.Providers.CommandCode;

public partial class CommandCodeProvider : IModelProvider
{
    ...
    public CommandCodeProvider(IApiKeyResolver keyResolver, AsyncCacheHelper asyncCacheHelper,
        IHttpClientFactory httpClientFactory)
    {
        _keyResolver = keyResolver;
        _memoryCache = asyncCacheHelper;
        _client = httpClientFactory.CreateClient();
        _client.BaseAddress = new Uri("https://api.commandcode.ai/provider");
    }
    ...
    private void ApplyAuthHeader()
    {
        ...
        if (string.IsNullOrWhiteSpace(key))
            throw new InvalidOperationException($"No {nameof(CommandCode)} API key.");
    ...
    public string GetIdentifier() => nameof(CommandCode).ToLowerInvariant();
