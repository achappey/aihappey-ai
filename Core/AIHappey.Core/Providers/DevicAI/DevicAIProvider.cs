...
public partial class DevicAIProvider : IModelProvider
{
    private readonly IApiKeyResolver _keyResolver;
    private readonly AsyncCacheHelper _memoryCache;
    private readonly HttpClient _client;

    public DevicAIProvider(
        ...
        IHttpClientFactory httpClientFactory)
    {
        ...
        _client.BaseAddress = new Uri("https://api.devic.ai/v1/");
        _client.Timeout = Timeout.InfiniteTimeSpan;
    }

    public string GetIdentifier() => "devicai";

    private void ApplyAuthHeader(HttpRequestMessage request)
    {
        var key = _keyResolver.Resolve(GetIdentifier());
        if (string.IsNullOrWhiteSpace(key))
            throw new InvalidOperationException("No DevicAI API key.");
