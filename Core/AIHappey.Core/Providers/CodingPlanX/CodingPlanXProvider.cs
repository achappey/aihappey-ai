...
namespace AIHappey.Core.Providers.CodingPlanX;

public partial class CodingPlanXProvider : IModelProvider
{
    ...
    public CodingPlanXProvider(IApiKeyResolver keyResolver, AsyncCacheHelper asyncCacheHelper,
        IHttpClientFactory httpClientFactory)
    {
        _keyResolver = keyResolver;
        _memoryCache = asyncCacheHelper;
        _client = httpClientFactory.CreateClient();
        _client.BaseAddress = new Uri("https://api.codingplanx.ai/");
    }
    ...
    private void ApplyAuthHeader()
    {
        ...
        if (string.IsNullOrWhiteSpace(key))
            throw new InvalidOperationException($"No {nameof(CodingPlanX)} API key.");
    ...
    public string GetIdentifier() => nameof(CodingPlanX).ToLowerInvariant();
