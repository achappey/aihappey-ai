...
public partial class MistralProvider : IModelProvider
{
    ...
    public async Task<ImageResponse> ImageRequest(ImageRequest imageRequest, CancellationToken cancellationToken = default)
    {
        ...
        var input = new
        {
            ...
            content = (object[])
                [
                    ...
                    ..imageRequest.Files?.Select(a =>
                        (object)new
                        {
                            type = "image_url",
                            image_url = a.Data.ToDataUrl(a.MediaType)
                        }) ?? [],
                    new { type = "text", text = imageRequest.Prompt },
                ]
        };
        ...
        var payload = new JsonObject
        {
            ...
            ["stream"] = false,
            ["store"] = false,
            ["inputs"] = JsonSerializer.SerializeToNode((object[])[input]),
        };
        ...
        {
            throw new Exception(resp.ReasonPhrase + body);
        }
