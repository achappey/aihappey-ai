...
public partial class AiMoProvider
{
        ...
        return await _memoryCache.GetOrCreateAsync(
            ...
            async ct =>
            {
                ...
                if (!resp.IsSuccessStatusCode)
                {
                    ...
                    throw new Exception($"AiMo API error: {err}");
                }
                ...
                foreach (var el in arr)
                {
                        ...
                        pricingEl.ValueKind == JsonValueKind.Object)
                    {
                        ...
                        if (!string.IsNullOrEmpty(promptPrice)
                            && !string.IsNullOrEmpty(completionPrice)
                            && promptPrice != "0"
                            && completionPrice != "0")
                        {
                            var inputPerMillion = decimal.Parse(promptPrice, CultureInfo.InvariantCulture);
                            var outputPerMillion = decimal.Parse(completionPrice, CultureInfo.InvariantCulture);
