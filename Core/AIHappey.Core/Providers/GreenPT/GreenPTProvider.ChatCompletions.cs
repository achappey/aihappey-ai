...
public partial class GreenPTProvider
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var payload = BuildGreenPTChatPayload(options, stream: false);
        ...
        if (!resp.IsSuccessStatusCode)
            throw new HttpRequestException($"GreenPT error: {(int)resp.StatusCode} {resp.ReasonPhrase}: {raw}");
        ...
        [EnumeratorCancellation] CancellationToken cancellationToken = default)
    {
        ...
        if (!resp.IsSuccessStatusCode)
        {
            ...
            throw new HttpRequestException($"GreenPT stream error: {(int)resp.StatusCode} {resp.ReasonPhrase}: {err}");
        }
    ...
    private static object BuildGreenPTChatPayload(ChatCompletionOptions options, bool stream)
    {
        var messages = options.Messages.ToGreenPTMessages().ToList();
