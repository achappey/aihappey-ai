...
namespace AIHappey.Core.Providers.MuleRun;

public partial class MuleRunProvider : IModelProvider
{
    ...
    private void ApplyAuthHeader()
    {
        ...
        if (string.IsNullOrWhiteSpace(key))
            throw new InvalidOperationException($"No {nameof(MuleRun)} API key.");
    ...
    public string GetIdentifier() => nameof(MuleRun).ToLowerInvariant();
    ...
    public async Task<ResponseResult> ResponsesAsync(ResponseRequest options, CancellationToken cancellationToken = default)
    {
        ...
        return await _client.GetResponses(
                   options,
                   relativeUrl: "vendors/openai/v1/responses",
