...
public partial class TinyFishProvider : IModelProvider
{
    private readonly IApiKeyResolver _keyResolver;

    private readonly HttpClient _client;

    public TinyFishProvider(IApiKeyResolver keyResolver, 
    ...
    private void ApplyAuthHeader()
    {
        ...
        if (string.IsNullOrWhiteSpace(key))
            throw new InvalidOperationException($"No {nameof(TinyFish)} API key.");
   ...
   public async Task<ChatCompletion> CompleteChatAsync(ChatCompletionOptions options, CancellationToken cancellationToken = default)
    ...
    public string GetIdentifier() => nameof(TinyFish).ToLowerInvariant();
