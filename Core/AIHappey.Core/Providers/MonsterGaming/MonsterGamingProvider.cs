...
namespace AIHappey.Core.Providers.MonsterGaming;

public partial class MonsterGamingProvider : IModelProvider
{
    ...
    public MonsterGamingProvider(IApiKeyResolver keyResolver, AsyncCacheHelper asyncCacheHelper,
        IHttpClientFactory httpClientFactory)
    {
        _keyResolver = keyResolver;
        _memoryCache = asyncCacheHelper;
        _client = httpClientFactory.CreateClient();
        _client.BaseAddress = new Uri("https://api.monstergaming.ai/");
    }
    ...
    private void ApplyAuthHeader()
    {
        ...
        if (string.IsNullOrWhiteSpace(key))
            throw new InvalidOperationException($"No {nameof(MonsterGaming)} API key.");
    ...
    public IAsyncEnumerable<ChatCompletionUpdate> CompleteChatStreamingAsync(ChatCompletionOptions options, CancellationToken cancellationToken = default)
    {
        ...
        return this.GetChatCompletions(_client,
                    options, 
                    relativeUrl:"v1/stream",
                    cancellationToken: cancellationToken);
    }

    public string GetIdentifier() => nameof(MonsterGaming).ToLowerInvariant();
