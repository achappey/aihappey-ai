...
public partial class RouterLinkProvider : IModelProvider
{
    ...
    public RouterLinkProvider(IApiKeyResolver keyResolver, AsyncCacheHelper asyncCacheHelper,
        IHttpClientFactory httpClientFactory)
    {
        _keyResolver = keyResolver;
        _memoryCache = asyncCacheHelper;
        _client = httpClientFactory.CreateClient();
        _client.BaseAddress = new Uri("https://router-link.world3.ai/api/");
    }
    ...
    private void ApplyAuthHeader()
    {
        ...
        if (string.IsNullOrWhiteSpace(key))
            throw new InvalidOperationException($"No {nameof(RouterLink)} API key.");
    ...
    public string GetIdentifier() => nameof(RouterLink).ToLowerInvariant();
