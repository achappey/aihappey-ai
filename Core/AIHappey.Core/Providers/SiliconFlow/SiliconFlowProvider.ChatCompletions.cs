...
public sealed partial class SiliconFlowProvider
{
    public async Task<ChatCompletion> CompleteChatAsync(ChatCompletionOptions options, CancellationToken cancellationToken)
    {
        ...
        if (!resp.IsSuccessStatusCode)
            throw new HttpRequestException($"SiliconFlow error: {(int)resp.StatusCode} {resp.ReasonPhrase}: {raw}");
        ...
        [System.Runtime.CompilerServices.EnumeratorCancellation] CancellationToken cancellationToken)
    {
        ...
        if (!resp.IsSuccessStatusCode)
        {
            ...
            throw new HttpRequestException($"SiliconFlowProvider stream error: {(int)resp.StatusCode} {resp.ReasonPhrase}: {err}");
        }
