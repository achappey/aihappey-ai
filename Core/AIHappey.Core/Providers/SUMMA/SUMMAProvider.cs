...
namespace AIHappey.Core.Providers.SUMMA;

public partial class SUMMAProvider : IModelProvider
{
    ...
    public SUMMAProvider(IApiKeyResolver keyResolver, AsyncCacheHelper asyncCacheHelper,
        IHttpClientFactory httpClientFactory)
    {
        _keyResolver = keyResolver;
        _memoryCache = asyncCacheHelper;
        _client = httpClientFactory.CreateClient();
        _client.BaseAddress = new Uri("https://alpha.backend.summachat.com/api/");
    }
    ...
    private void ApplyAuthHeader()
    {
        ...
        if (string.IsNullOrWhiteSpace(key))
            throw new InvalidOperationException($"No {nameof(SUMMA)} API key.");
    ...
    public string GetIdentifier() => nameof(SUMMA).ToLowerInvariant();
