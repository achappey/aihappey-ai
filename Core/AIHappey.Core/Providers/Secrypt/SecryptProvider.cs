...
namespace AIHappey.Core.Providers.Secrypt;

public partial class SecryptProvider : IModelProvider
{
    ...
    public SecryptProvider(IApiKeyResolver keyResolver, AsyncCacheHelper asyncCacheHelper,
        IHttpClientFactory httpClientFactory)
    {
        _keyResolver = keyResolver;
        _memoryCache = asyncCacheHelper;
        _client = httpClientFactory.CreateClient();
        _client.BaseAddress = new Uri("https://secrypt.space/");
    }
    ...
    private void ApplyAuthHeader()
    {
        ...
        if (string.IsNullOrWhiteSpace(key))
            throw new InvalidOperationException($"No {nameof(Secrypt)} API key.");
    ...
    public string GetIdentifier() => nameof(Secrypt).ToLowerInvariant();
