...
public partial class EmpirioLabsAIProvider : IModelProvider
{
    ...
    public EmpirioLabsAIProvider(IApiKeyResolver keyResolver, AsyncCacheHelper asyncCacheHelper,
        IHttpClientFactory httpClientFactory)
    {
        _keyResolver = keyResolver;
        _memoryCache = asyncCacheHelper;
        _client = httpClientFactory.CreateClient();
        _client.BaseAddress = new Uri("https://api.empiriolabs.ai/");
    ...
    private void ApplyAuthHeader()
    {
        ...
        if (string.IsNullOrWhiteSpace(key))
            throw new InvalidOperationException($"No {nameof(EmpirioLabsAI)} API key.");
    ...
    public string GetIdentifier() => nameof(EmpirioLabsAI).ToLowerInvariant();
    ...
    public Task<AIResponse> ExecuteUnifiedAsync(AIRequest request, CancellationToken cancellationToken = default)
      => IsEmpirioAgentModel(request.Model)
          ? ExecuteEmpirioAgentUnifiedAsync(request, cancellationToken)
          : this.ExecuteUnifiedViaChatCompletionsAsync(request, cancellationToken: cancellationToken);

    public IAsyncEnumerable<AIStreamEvent> StreamUnifiedAsync(AIRequest request, CancellationToken cancellationToken = default)
        => IsEmpirioAgentModel(request.Model)
            ? StreamEmpirioAgentUnifiedAsync(request, cancellationToken)
