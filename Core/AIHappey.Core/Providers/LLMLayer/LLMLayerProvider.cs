...
public partial class LLMLayerProvider : IModelProvider
{
    ...
    public LLMLayerProvider(IApiKeyResolver keyResolver, IHttpClientFactory httpClientFactory)
    {
        _keyResolver = keyResolver;
        _client = httpClientFactory.CreateClient();
        _client.BaseAddress = new Uri("https://api.llmlayer.dev/");
    }
    ...
    private void ApplyAuthHeader()
    {
        ...
        if (string.IsNullOrWhiteSpace(key))
            throw new InvalidOperationException($"No {nameof(LLMLayer)} API key.");
    ...
    public string GetIdentifier() => nameof(LLMLayer).ToLowerInvariant();
