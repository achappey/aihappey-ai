...
namespace AIHappey.Core.Providers.SailResearch;

public partial class SailResearchProvider : IModelProvider
{
    ...
    public SailResearchProvider(IApiKeyResolver keyResolver, AsyncCacheHelper asyncCacheHelper,
        IHttpClientFactory httpClientFactory)
    {
        _keyResolver = keyResolver;
        _memoryCache = asyncCacheHelper;
        _client = httpClientFactory.CreateClient();
        _client.BaseAddress = new Uri("https://api.sailresearch.com/");
    }
    ...
    private void ApplyAuthHeader()
    {
        ...
        if (string.IsNullOrWhiteSpace(key))
            throw new InvalidOperationException($"No {nameof(SailResearch)} API key.");
    ...
    public string GetIdentifier() => nameof(SailResearch).ToLowerInvariant();
