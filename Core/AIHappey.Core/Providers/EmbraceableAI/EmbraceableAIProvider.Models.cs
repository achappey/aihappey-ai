    ...
    public async Task<IEnumerable<Model>> ListModels(CancellationToken cancellationToken = default)
    {
        ...
        return await _memoryCache.GetOrCreateAsync(
            ...
            async ct =>
            {
                ...
                using var req = new HttpRequestMessage(HttpMethod.Get, "api/preview/model");
                using var resp = await _client.SendAsync(req, cancellationToken);

                if (!resp.IsSuccessStatusCode)
                {
                    var err = await resp.Content.ReadAsStringAsync(cancellationToken);
                    throw new Exception($"EmbraceableAI API error: {err}");
                }
                ...
                foreach (var el in arr)
                {
                    ...
                    if (el.TryGetProperty("description", out var orgEl))
                        model.Description = orgEl.GetString() ?? string.Empty;
