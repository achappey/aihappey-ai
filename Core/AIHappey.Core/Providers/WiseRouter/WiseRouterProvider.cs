...
namespace AIHappey.Core.Providers.WiseRouter;

public partial class WiseRouterProvider : IModelProvider
{
    ...
    public WiseRouterProvider(IApiKeyResolver keyResolver, AsyncCacheHelper asyncCacheHelper,
        IHttpClientFactory httpClientFactory)
    {
        _keyResolver = keyResolver;
        _memoryCache = asyncCacheHelper;
        _client = httpClientFactory.CreateClient();
        _client.BaseAddress = new Uri("https://api.wiserouter.ai/");
    }
    ...
    private void ApplyAuthHeader()
    {
        ...
        if (string.IsNullOrWhiteSpace(key))
            throw new InvalidOperationException($"No {nameof(WiseRouter)} API key.");
    ...
    private static void ApplyRouterPreference(ChatCompletionOptions options)
    {
        ...
        options.AdditionalProperties["router_preference"] =
    ...
    public IAsyncEnumerable<ChatCompletionUpdate> CompleteChatStreamingAsync(
        ChatCompletionOptions options,
        CancellationToken cancellationToken = default)
    {
        ApplyAuthHeader();
    ...
    public string GetIdentifier() => nameof(WiseRouter).ToLowerInvariant();
