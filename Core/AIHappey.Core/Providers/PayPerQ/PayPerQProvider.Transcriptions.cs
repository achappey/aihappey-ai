...
public partial class PayPerQProvider
{
    public async Task<TranscriptionResponse> TranscriptionRequest(TranscriptionRequest request, CancellationToken cancellationToken = default)
    {
        ...
        var fields = CopyPayPerQOptions(request.ProviderOptions);
        fields["response_format"] = "verbose_json";
        var result = await SendPayPerQTranscriptionAsync(DecodePayPerQBase64(request.Audio), request.MediaType,
            PayPerQAudioFileName(request.MediaType), request.Model, fields, cancellationToken);
        var segments = ReadPayPerQSegments(result.Root);
        return new TranscriptionResponse
        {
            Text = PayPerQGetString(result.Root, "text") ?? string.Join(" ", segments.Select(x => x.Text)),
            Language = PayPerQGetString(result.Root, "language"),
            DurationInSeconds = PayPerQGetFloat(result.Root, "duration"),
            Segments = segments,
        ...
        StreamingTranscriptionRequest request, [EnumeratorCancellation] CancellationToken cancellationToken = default)
    {
        ...
        var response = await TranscriptionRequest(new TranscriptionRequest
        {
            ...
            MediaType = ResolvePayPerQAudioMediaType(request.InputAudioFormat?.Type),
        ...
        OpenAITranscriptionRequest options, CancellationToken cancellationToken = default)
    {
        ...
        var result = await SendPayPerQTranscriptionAsync(memory.ToArray(), options.File.ContentType,
        ...
        if (string.Equals(format, "verbose_json", StringComparison.OrdinalIgnoreCase))
            return JsonSerializer.Deserialize<OpenAITranscriptionVerboseResponse>(result.Root.GetRawText())
                ?? throw new InvalidOperationException("PayPerQ returned an invalid verbose transcription response.");
        return JsonSerializer.Deserialize<OpenAITranscriptionResponse>(result.Root.GetRawText())
            ?? throw new InvalidOperationException("PayPerQ returned an invalid transcription response.");
    }
    ...
    private async Task<PayPerQTranscriptionResult> SendPayPerQTranscriptionAsync(byte[] audio, string? mediaType,
        string? fileName, string model, Dictionary<string, object?> fields, CancellationToken cancellationToken)
    {
        ...
        foreach (var (name, value) in fields)
        {
            ...
            if (value is string[] strings)
                ...
                foreach (var item in element.EnumerateArray()) form.Add(new StringContent(PayPerQJsonText(item)), name.EndsWith("[]") ? name : name + "[]");
            else form.Add(new StringContent(value is JsonElement json ? PayPerQJsonText(json)
        ...
        EnsurePayPerQSuccess(response, raw, "transcription request");
        var requestedFormat = fields.TryGetValue("response_format", out var format) ? format?.ToString() : null;
        JsonElement root;
        if (requestedFormat?.ToLowerInvariant() is "text" or "srt" or "vtt")
        ...
        return new PayPerQTranscriptionResult(root, response.GetHeaders());
    }

    private static List<TranscriptionSegment> ReadPayPerQSegments(JsonElement root)
        => root.TryGetProperty("segments", out var value) && value.ValueKind == JsonValueKind.Array
            ...
            {
                Text = PayPerQGetString(x, "text") ?? string.Empty,
                StartSecond = PayPerQGetFloat(x, "start") ?? 0,
                EndSecond = PayPerQGetFloat(x, "end") ?? 0
            }).ToList() : [];

    private static string ResolvePayPerQAudioMediaType(string? type) => type?.Trim().ToLowerInvariant() switch
    {
        "mp3" or "mpeg" or "mpga" => "audio/mpeg", "mp4" => "audio/mp4", "m4a" => "audio/m4a",
        "wav" => "audio/wav", "webm" => "audio/webm", _ => "application/octet-stream"
    };

    private static string PayPerQAudioFileName(string mediaType) => "audio" + mediaType.ToLowerInvariant() switch
    {
        "audio/mpeg" => ".mp3", "audio/mp4" => ".mp4", "audio/m4a" => ".m4a",
        "audio/wav" => ".wav", "audio/webm" => ".webm", _ => ".bin"
    };

    private sealed record PayPerQTranscriptionResult(JsonElement Root, Dictionary<string, string> Headers);
}
