        ...
        return await _memoryCache.GetOrCreateAsync(
            ...
            async ct =>
            {
                ...
                if (!resp.IsSuccessStatusCode)
                {
                    ...
                    throw new Exception($"PayPerQ API error: {err}");
                }
                ...
                foreach (var el in arr)
                {
                    ...
                    {
                        var inputPrice = pricingEl.TryGetProperty("input_per_1M_tokens", out var inEl)
                            ? inEl.GetDecimal() / 1_000_000m
                            : (decimal?)null;

                        var outputPrice = pricingEl.TryGetProperty("output_per_1M_tokens", out var outEl)
                            ? outEl.GetDecimal() / 1_000_000m
                            : (decimal?)null;
