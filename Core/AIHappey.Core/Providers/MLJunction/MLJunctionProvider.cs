...
namespace AIHappey.Core.Providers.MLJunction;

public partial class MLJunctionProvider : IModelProvider
{
    ...
    public MLJunctionProvider(IApiKeyResolver keyResolver, AsyncCacheHelper asyncCacheHelper,
        IHttpClientFactory httpClientFactory)
    {
        _keyResolver = keyResolver;
        _memoryCache = asyncCacheHelper;
        _client = httpClientFactory.CreateClient();
        _client.BaseAddress = new Uri("https://api.mljunction.com/");
    }
    ...
    private void ApplyAuthHeader()
    {
        ...
        if (string.IsNullOrWhiteSpace(key))
            throw new InvalidOperationException($"No {nameof(MLJunction)} API key.");
    ...
    public string GetIdentifier() => nameof(MLJunction).ToLowerInvariant();
