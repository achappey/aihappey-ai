...
public partial class DandoloProvider : IModelProvider
{
    private readonly IApiKeyResolver _keyResolver;

    private readonly HttpClient _client;

    public DandoloProvider(IApiKeyResolver keyResolver, IHttpClientFactory httpClientFactory)
    {
    ...
    private void ApplyAuthHeader()
    {
        ...
        if (string.IsNullOrWhiteSpace(key))
            throw new InvalidOperationException($"No {nameof(Dandolo)} API key.");
    ...
    public string GetIdentifier() => nameof(Dandolo).ToLowerInvariant();
