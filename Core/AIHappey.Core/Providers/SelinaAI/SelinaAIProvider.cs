...
namespace AIHappey.Core.Providers.SelinaAI;

public partial class SelinaAIProvider : IModelProvider
{
    ...
    public SelinaAIProvider(IApiKeyResolver keyResolver, AsyncCacheHelper asyncCacheHelper,
        IHttpClientFactory httpClientFactory)
    {
        _keyResolver = keyResolver;
        _memoryCache = asyncCacheHelper;
        _client = httpClientFactory.CreateClient();
        _client.BaseAddress = new Uri("https://api.selina.ai/");
    }
    ...
    private void ApplyAuthHeader()
    {
        ...
        if (string.IsNullOrWhiteSpace(key))
            throw new InvalidOperationException($"No {nameof(SelinaAI)} API key.");
    ...
    public string GetIdentifier() => nameof(SelinaAI).ToLowerInvariant();
  ...
  => this.ExecuteUnifiedViaChatCompletionsAsync(request, cancellationToken: cancellationToken, true);
