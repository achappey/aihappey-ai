...
public partial class VibeKitProvider : IModelProvider
{
    ...
    public VibeKitProvider(IApiKeyResolver keyResolver, AsyncCacheHelper asyncCacheHelper,
        IHttpClientFactory httpClientFactory)
    {
        _keyResolver = keyResolver;
        _memoryCache = asyncCacheHelper;
        _client = httpClientFactory.CreateClient();
        _client.BaseAddress = new Uri("https://vibekit.bot/");
    }
    ...
    private void ApplyAuthHeader()
    {
        ...
        if (string.IsNullOrWhiteSpace(key))
            throw new InvalidOperationException($"No {nameof(VibeKit)} API key.");
    ...
    public string GetIdentifier() => nameof(VibeKit).ToLowerInvariant();
