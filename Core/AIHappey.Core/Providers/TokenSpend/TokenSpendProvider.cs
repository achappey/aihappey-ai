...
namespace AIHappey.Core.Providers.TokenSpend;

public partial class TokenSpendProvider : IModelProvider
{
    ...
    public TokenSpendProvider(IApiKeyResolver keyResolver, AsyncCacheHelper asyncCacheHelper,
        IHttpClientFactory httpClientFactory)
    {
        _keyResolver = keyResolver;
        _memoryCache = asyncCacheHelper;
        _client = httpClientFactory.CreateClient();
        _client.BaseAddress = new Uri("https://api.tokenspend.dev/");
    }
    ...
    private void ApplyAuthHeader()
    {
        ...
        if (string.IsNullOrWhiteSpace(key))
            throw new InvalidOperationException($"No {nameof(TokenSpend)} API key.");
    ...
    public string GetIdentifier() => nameof(TokenSpend).ToLowerInvariant();
