...
public partial class EphoProvider : IModelProvider
{
    ...
    public EphoProvider(IApiKeyResolver keyResolver, AsyncCacheHelper asyncCacheHelper,
        IHttpClientFactory httpClientFactory)
    {
        _keyResolver = keyResolver;
        _memoryCache = asyncCacheHelper;
        _client = httpClientFactory.CreateClient();
        _client.BaseAddress = new Uri("https://app.epho.io/api/v1/");
    }
    ...
    private void ApplyAuthHeader(HttpRequestMessage request)
    {
        ...
        if (string.IsNullOrWhiteSpace(key))
            throw new InvalidOperationException($"No {nameof(Epho)} API key.");
    ...
    public string GetIdentifier() => nameof(Epho).ToLowerInvariant();
