...
public partial class EmberCloudProvider : IModelProvider
{
    ...
    public EmberCloudProvider(IApiKeyResolver keyResolver, AsyncCacheHelper asyncCacheHelper,
        IHttpClientFactory httpClientFactory)
    {
        _keyResolver = keyResolver;
        _memoryCache = asyncCacheHelper;
        _client = httpClientFactory.CreateClient();
        _client.BaseAddress = new Uri("https://api.embercloud.ai/");
    }
    ...
    private void ApplyAuthHeader()
    {
        ...
        if (string.IsNullOrWhiteSpace(key))
            throw new InvalidOperationException($"No {nameof(EmberCloud)} API key.");
    ...
    public string GetIdentifier() => nameof(EmberCloud).ToLowerInvariant();
