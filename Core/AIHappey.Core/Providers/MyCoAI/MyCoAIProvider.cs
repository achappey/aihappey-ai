...
public partial class MyCoAIProvider : IModelProvider
{
    ...
    public MyCoAIProvider(IApiKeyResolver keyResolver, AsyncCacheHelper asyncCacheHelper,
        IHttpClientFactory httpClientFactory)
    {
        _keyResolver = keyResolver;
        _memoryCache = asyncCacheHelper;
        _client = httpClientFactory.CreateClient();
        _client.BaseAddress = new Uri("https://mycoai.dev/");
    }
    ...
    private void ApplyAuthHeader()
    {
        ...
        if (string.IsNullOrWhiteSpace(key))
            throw new InvalidOperationException($"No {nameof(MyCoAI)} API key.");
    ...
    public string GetIdentifier() => nameof(MyCoAI).ToLowerInvariant();
    ...
    public Task<AIResponse> ExecuteUnifiedAsync(AIRequest request, CancellationToken cancellationToken = default)
      => request.Model?.StartsWith("claude", StringComparison.OrdinalIgnoreCase) == true
