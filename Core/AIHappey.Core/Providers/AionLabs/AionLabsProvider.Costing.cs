...
public partial class AionLabsProvider
{
    ...
    public static AIResponse EnrichUnifiedResponseWithGatewayCostForTests(
        AIResponse response,
        ModelPricing? pricing)
        => EnrichUnifiedResponseWithGatewayCost(response, pricing);
        ...
        CancellationToken cancellationToken)
    {
                     ...
                     .Distinct(StringComparer.OrdinalIgnoreCase))
        {
            ...
            try
            {
                model = await this.GetModel(candidate, cancellationToken);
            }
    ...
    {
        var pricing = await ResolveModelListingPricingAsync(response.Model, requestModel, cancellationToken);
        return EnrichUnifiedResponseWithGatewayCost(response, pricing);
    }

    private static AIResponse EnrichUnifiedResponseWithGatewayCost(
        AIResponse response,
        ModelPricing? pricing)
    {
        ...
        ModelPricing? pricing)
    {
        ...
        var cachedInputReadTokens = TryGetUsageInt(
            finishData.MessageMetadata?.Usage,
            "cachedInputTokens",
            "cached_input_tokens",
        ...
        if (cachedInputReadTokens == 0)
            cachedInputReadTokens = TryGetNestedUsageInt(
                finishData.MessageMetadata?.Usage,
    ...
    private static int? TryGetNestedUsageInt(JsonElement? usage, string objectName, string propertyName)
    {
        if (usage is not { ValueKind: JsonValueKind.Object } usageElement
            || !usageElement.TryGetProperty(objectName, out var nested)
            || nested.ValueKind != JsonValueKind.Object)
        {
            return null;
        }

        return TryGetUsageInt(nested, propertyName);
    }
    ...
    private static IEnumerable<string> GetSingleModelPricingLookupCandidates(string? modelId)
    {
        ...
        if (!trimmed.StartsWith("aionlabs/", StringComparison.OrdinalIgnoreCase))
            yield return $"aionlabs/{trimmed}";

        if (trimmed.StartsWith("aion-labs/", StringComparison.OrdinalIgnoreCase))
            yield return trimmed["aion-labs/".Length..];
        ...
        if (!split.Model.StartsWith("aionlabs/", StringComparison.OrdinalIgnoreCase))
            yield return $"aionlabs/{split.Model}";

        if (split.Model.StartsWith("aion-labs/", StringComparison.OrdinalIgnoreCase))
            yield return split.Model["aion-labs/".Length..];
    }
