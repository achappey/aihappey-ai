...
public sealed partial class PloriProvider : IModelProvider, IUnifiedModelProvider
{
    private readonly IApiKeyResolver _keys;
    private readonly AsyncCacheHelper _cache;
    private readonly HttpClient _client;

    public PloriProvider(IApiKeyResolver keys, AsyncCacheHelper cache, IHttpClientFactory clients)
    {
        _keys = keys;
        _cache = cache;
        _client = clients.CreateClient();
        _client.BaseAddress = new Uri("https://api.plori.ai/v1/");
    }

    public string GetIdentifier() => "plori";

    private string Key() => _keys.Resolve(GetIdentifier()) is { Length: > 0 } key
        ? key : throw new InvalidOperationException("No Plori API key.");
