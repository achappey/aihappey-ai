...
namespace AIHappey.Core.Providers.OpenLimits;

public partial class OpenLimitsProvider : IModelProvider
{
    ...
    public OpenLimitsProvider(IApiKeyResolver keyResolver, AsyncCacheHelper asyncCacheHelper,
        IHttpClientFactory httpClientFactory)
    {
        _keyResolver = keyResolver;
        _memoryCache = asyncCacheHelper;
        _client = httpClientFactory.CreateClient();
        _client.BaseAddress = new Uri("https://openlimits.app/");
    }
    ...
    private void ApplyAuthHeader()
    {
        ...
        if (string.IsNullOrWhiteSpace(key))
            throw new InvalidOperationException($"No {nameof(OpenLimits)} API key.");
    ...
    public string GetIdentifier() => nameof(OpenLimits).ToLowerInvariant();
