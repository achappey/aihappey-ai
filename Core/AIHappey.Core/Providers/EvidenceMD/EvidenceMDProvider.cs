...
namespace AIHappey.Core.Providers.EvidenceMD;

public partial class EvidenceMDProvider : IModelProvider
{
    private readonly IApiKeyResolver _keyResolver;

    private readonly HttpClient _client;

    public EvidenceMDProvider(IApiKeyResolver keyResolver,
        IHttpClientFactory httpClientFactory)
    {
        _keyResolver = keyResolver;
        _client = httpClientFactory.CreateClient();
        _client.BaseAddress = new Uri("https://evidencemd.ai/api/");
    }
    ...
    private void ApplyAuthHeader()
    {
        ...
        if (string.IsNullOrWhiteSpace(key))
            throw new InvalidOperationException($"No {nameof(EvidenceMD)} API key.");
    ...
    public string GetIdentifier() => nameof(EvidenceMD).ToLowerInvariant();
