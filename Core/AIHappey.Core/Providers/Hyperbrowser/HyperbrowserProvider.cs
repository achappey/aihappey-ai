...
public partial class HyperbrowserProvider : IModelProvider
{
    ...
    public HyperbrowserProvider(IApiKeyResolver keyResolver, AsyncCacheHelper asyncCacheHelper,
        IHttpClientFactory httpClientFactory)
    {
        _keyResolver = keyResolver;
        _memoryCache = asyncCacheHelper;
        _client = httpClientFactory.CreateClient();
        _client.BaseAddress = new Uri("https://api.hyperbrowser.ai/api/");
    }
    ...
    private void ApplyAuthHeader()
    {
        ...
        if (string.IsNullOrWhiteSpace(key))
            throw new InvalidOperationException($"No {nameof(Hyperbrowser)} API key.");
    ...
    public string GetIdentifier() => nameof(Hyperbrowser).ToLowerInvariant();
