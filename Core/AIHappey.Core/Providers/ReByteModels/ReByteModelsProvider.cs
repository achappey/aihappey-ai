...
namespace AIHappey.Core.Providers.ReByteModels;

public partial class ReByteModelsProvider : IModelProvider
{
    private readonly IApiKeyResolver _keyResolver;

    private readonly HttpClient _client;

    public ReByteModelsProvider(IApiKeyResolver keyResolver, 
        IHttpClientFactory httpClientFactory)
    {
        _keyResolver = keyResolver;
        _client = httpClientFactory.CreateClient();
        _client.BaseAddress = new Uri("https://model.rebyte.ai/");
    }
    ...
    private void ApplyAuthHeader()
    {
        ...
        if (string.IsNullOrWhiteSpace(key))
            throw new InvalidOperationException($"No {nameof(ReByteModels)} API key.");
    ...
    public string GetIdentifier() => nameof(ReByteModels).ToLowerInvariant();
