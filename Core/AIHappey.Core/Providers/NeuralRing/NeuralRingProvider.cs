...
namespace AIHappey.Core.Providers.NeuralRing;

public partial class NeuralRingProvider : IModelProvider
{
    ...
    public NeuralRingProvider(IApiKeyResolver keyResolver, AsyncCacheHelper asyncCacheHelper,
        IHttpClientFactory httpClientFactory)
    {
        _keyResolver = keyResolver;
        _memoryCache = asyncCacheHelper;
        _client = httpClientFactory.CreateClient();
        _client.BaseAddress = new Uri("https://neuralring.eu/api/");
    }
    ...
    private void ApplyAuthHeader()
    {
        ...
        if (string.IsNullOrWhiteSpace(key))
            throw new InvalidOperationException($"No {nameof(NeuralRing)} API key.");
    ...
    public string GetIdentifier() => nameof(NeuralRing).ToLowerInvariant();
