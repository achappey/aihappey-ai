...
{
    private async Task<SpeechResponse> TextToSoundRequest(SpeechRequest request, CancellationToken cancellationToken)
    {
        ...
        var url = "v1/sound-generation" + (query.Count > 0 ? "?" + string.Join("&", query) : string.Empty);

        // Model-id mapping: accept both "elevenlabs/<model>" and "<model>".
        var warnings = new List<object>();
        // Explicitly ignored/unsupported in text-to-sound mode.
        if (!string.IsNullOrWhiteSpace(request.Voice))
            warnings.Add(new { type = "ignored", feature = "voice" });
        if (!string.IsNullOrWhiteSpace(request.Language))
            warnings.Add(new { type = "ignored", feature = "language" });
        ...
        var body = new Dictionary<string, object?>
        {
            ...
            ["loop"] = metadata?.TextToSound?.Loop,
            ["duration_seconds"] = metadata?.TextToSound?.DurationSeconds,
            ["prompt_influence"] = metadata?.TextToSound?.PromptInfluence,
        };
        ...
        if (!resp.IsSuccessStatusCode)
            throw new InvalidOperationException($"ElevenLabs Text-to-Sound failed ({(int)resp.StatusCode}): {Encoding.UTF8.GetString(bytes)}");
