...
public partial class BlockRunProvider : IModelProvider
{
    ...
    public BlockRunProvider(IApiKeyResolver keyResolver, AsyncCacheHelper asyncCacheHelper,
        IHttpClientFactory httpClientFactory)
    {
        _keyResolver = keyResolver;
        _memoryCache = asyncCacheHelper;
        _client = httpClientFactory.CreateClient();
        _client.BaseAddress = new Uri("https://blockrun.ai/api/");
    }
    ...
    private void ApplyAuthHeader()
    {
        ...
        if (string.IsNullOrWhiteSpace(key))
            throw new InvalidOperationException($"No {nameof(BlockRun)} API key.");
    ...
    public string GetIdentifier() => nameof(BlockRun).ToLowerInvariant();
